...
Filename convention:
  {material}_{slot}_{variant}{N}_tier{T}.png   e.g. wooden_sword_large3_tier1.png
  {material}_{slot}_type{N}_tier{T}.png        e.g. rings_type1_tier1.png
  {material}_{slot}.png                        e.g. shadow_boots.png
...
# ─────────────────────────────────────────────────────────────────────────────
# SLOT KEYWORDS  —  keyword in filename stem → canonical slot name
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# STAT WEIGHTS  —  per slot: (damage_w, Health_w, defense_w, move_w)
# Weapon weights are overridden at runtime based on variant (large/slim)
# ─────────────────────────────────────────────────────────────────────────────

SLOT_STAT_WEIGHTS = {
    "Weapon":  (1.0,  0.0,  0.0,  0.0),   # overridden by variant logic below
...
# ─────────────────────────────────────────────────────────────────────────────
# WEAPON VARIANTS  —  keyword → (damage_weight, attack_speed_weight)
# large/heavy = more damage, slower swing
# slim/light  = less damage, faster swing
# default     = balanced
# ─────────────────────────────────────────────────────────────────────────────
...
WEAPON_DEFAULT_ATK_SPEED_W = 1.0

# ─────────────────────────────────────────────────────────────────────────────
# BASE STATS  —  values at type level 1, before scaling
# ─────────────────────────────────────────────────────────────────────────────
...
BASE_STATS = {
    ...
    "defense":      (2.0,  6.0),
    "move":         (1.0,  3.0),
    "attack_speed": (0.8,  1.2),   # multiplier; 1.0 = normal swing speed
}
...
# ─────────────────────────────────────────────────────────────────────────────
# MATERIAL → type level  (used when no explicit typeN in filename)
# ─────────────────────────────────────────────────────────────────────────────
...
MATERIAL_TIER = {
    ...
    "void":       8,
    "divine":     9,
    # future fantasy materials — add more here as needed
    "adamantite": 7,
    "mithril":    6,
    "dragonite":  8,
    "runic":      7,
...
def infer_weapon_variant(stem: str):
    """
    For weapon filenames, detect variant keyword.
    e.g. 'wooden_sword_large3_tier1' → ('large', 1.5, 0.6)
    Returns (variant_name, dmg_w, atk_spd_w) or (None, 1.0, 1.0) for balanced.
    """
    # Strip trailing digits from each part so 'large3' matches 'large'
    parts = stem.lower().replace("-", "_").split("_")
    stripped = [re.sub(r"\d+$", "", p) for p in parts]
    for part in stripped:
        if part in WEAPON_VARIANTS:
            dmg_w, spd_w = WEAPON_VARIANTS[part]
            return part, dmg_w, spd_w
    return None, 1.0, WEAPON_DEFAULT_ATK_SPEED_W


def infer_type_level(stem: str) -> int:
    """Return the type number (1-9+) encoded in the filename."""
    # explicit "typeN" or "largeN" / "slimN" style numbering
...
def compute_stat_range(base_min, base_max, type_level, weight):
    """Scale a stat range by type level and weight. Returns (0, 0) if weight is 0."""
    if weight == 0:
        return 0.0, 0.0
...
def build_row(next_id, stem, slot, icon_path) -> dict:
    """Compute all stats and return a fully populated CSV row dict."""
    type_level = infer_type_level(stem)
    weights    = SLOT_STAT_WEIGHTS.get(slot, (0, 0, 0, 0))
    dmg_w, Health_w, def_w, move_w = weights

    # ── weapon variant overrides ──────────────────────────────────────────────
    atk_spd_min, atk_spd_max = 0.0, 0.0
    if slot == "Weapon":
        variant, dmg_w, spd_w = infer_weapon_variant(stem)
        atk_spd_min, atk_spd_max = compute_stat_range(
            *BASE_STATS["attack_speed"], type_level, spd_w)
    ...
    return {
        ...
        "attack_speed_min":    atk_spd_min,
        "attack_speed_max":    atk_spd_max,
    }
...
def main():
    ...
    for filename in png_files:
        ...
        row = build_row(next_id, stem, slot, icon_path)
        new_rows.append(row)
        next_id += 1
    ...
    # ── preview ────────────────────────────────────────────────────────────────
    print("\nNew rows preview:")
    header = f"  {'id':>4}  {'name':<42}  {'slot':<8}  {'lvl':>3}  {'dmg':^9}  {'hp':^9}  {'def':^9}  {'mov':^9}  {'spd':^9}"
    print(header)
    print("  " + "-" * (len(header) - 2))
    for r in new_rows:
        lvl = infer_type_level(r["name"])
        variant, _, _ = infer_weapon_variant(r["name"]) if r["slot"] == "Weapon" else (None, 0, 0)
        variant_tag = f" [{variant}]" if variant else ""
        print(
            f"  {r['id']:>4}  {r['name'] + variant_tag:<42}  {r['slot']:<8}  {lvl:>3}  "
            ...
            f"{r['attack_speed_min']:>4}-{r['attack_speed_max']:<4}"
        )
